...
exec /opt/pupil_service/pupil_service "$@"'''
        f.write(content)
    os.chmod(os.path.join(bin_dir,'pupil_service'),0755)
...
Exec=x-terminal-emulator -e pupil_service'''
        f.write(content)
    os.chmod(os.path.join(app_dir,'pupil_service.desktop'),0644)

    #copy icon:
    shutil.copy('pupil-service.svg',ico_dir)
    os.chmod(os.path.join(ico_dir,'pupil-service.svg'),0644)
