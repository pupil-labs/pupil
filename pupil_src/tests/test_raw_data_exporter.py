...
def _test_exporter(exporter, positions, expected_dict_export, world_index=123):
    ...
    assert set(sample_labels) == set(actual_dict_export.keys()), "Labels must be the keys for the exported dict"
    assert actual_dict_export == expected_dict_export, "Actual pupil export must be the same as expeted export"
...
PUPIL_CAPTURE_GAZE_POSITION_0 = {
    ...
    'base_data': [
        ...
        },
        {
            'topic': 'pupil.1',
            'circle_3d': {
                'center': (-1.8241334685531108, -4.1069075704320746, 101.41643695709374),
                'normal': (-0.008068882229597849, -0.30213665999147843, -0.9532304715171239),
