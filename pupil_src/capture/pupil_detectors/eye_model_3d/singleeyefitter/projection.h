...
namespace singleeyefitter {
    ...
    Ellipse2D<Scalar> project_circle_to_ellipse(const Circle3D<Scalar>& circle, Eigen::Matrix<Scalar, 3, 3> intrinsics){
        typedef typename Circle3D<Scalar>::Vector Vector;
        using math::sq;
        ...
        // Then set z=f to get conic which is the result of intersecting the cone with the focal plane
        Conic<Scalar> temp_conic = Conic<Scalar>(
            ABC(0), // x^2 (Ax^2)
            H, // xy (Hxy)
            ABC(1), // y^2 (By^2)
            G*f /*+ Const(0)*/, // x (Gxz + Ux, z = f)
    ...
    Ellipse2D<Scalar> project_sphere(const Sphere<Scalar>& sphere, Eigen::Matrix<Scalar, 3, 3> intrinsics){
        auto center = project_point(sphere.center, intrinsics);
