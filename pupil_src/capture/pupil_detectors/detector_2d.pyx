...
cdef extern from 'detect_2d.hpp':
  ...
  cdef cppclass Detector2D:
    ...
    Detector2D() except +
    Result detect( Mat& image, Rect_[int]& usr_roi, Mat& color_image , bint visualize , DetectProperties& prop )
...
cdef class Detector_2D:
    ...
    def detect(self, frame, roi, visualization ):
        ...
        result =  self.thisptr.detect( cv_image , Rect_[int](x,y,width,height), cv_image_color, True , detect_properties)
        return result
