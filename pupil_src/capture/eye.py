...
def eye(g_pool,cap_src,cap_size,rx_from_world,eye_id=0):
    ...
    g_pool.capture = cap
    g_pool.h_flip = session_settings.get('h_flip',False)
    ...
    general_settings.append(ui.Switch('h_flip',g_pool))
    ...
    while not g_pool.quit.value:
        ...
        make_coord_system_norm_based(g_pool.h_flip)
        draw_named_texture(g_pool.image_tex)
        # switch to work in pixel space
        make_coord_system_pixel_based((frame.height,frame.width,3),g_pool.h_flip)
    ...
    session_settings['h_flip'] = g_pool.h_flip
