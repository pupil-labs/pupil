...
def bin_thresholding(image, image_lower=0, image_upper=256):
    binary_img = cv2.inRange(image, np.asarray(image_lower), np.asarray(image_upper))
...
def split_at_angle(contour, curvature, angle):
    ...
    for s, e in zip(
        [0] + kink_index, kink_index + [None]
    ):  # list of slice indecies 0,i0,i1,i2,None
        if e is not None:
...
def find_kink_and_dir_change(curvature, angle):
    ...
    for idx, c, is_posisitve in zip(
        range(curvature.shape[0]), curvature, curv_positive
    ):
...
def split_at_corner_index(contour, index):
    ...
    for s, e in zip(
        [0] + index, index + [10000000]
    ):  # list of slice indecies 0,i0,i1,i2,
...
def convexity_defect(contour, curvature):
    ...
    for s in kink_index:  # list of slice indecies 0,i0,i1,i2,None
        kinks.append(
            contour[s + 1]
        )  # because the curvature is n-2 (1st and last are not exsistent)
...
def is_round(ellipse, ratio, tolerance=0.8):
    ...
    if (
        axis1
        and axis2
        and abs(ratio - min(axis2, axis1) / max(axis2, axis1)) < tolerance
    ):
        return True
    else:
        return False
...
def circle_grid(image, pattern_size=(4, 11)):
    ...
    """
    status, centers = cv2.findCirclesGridDefault(
        image, pattern_size, flags=cv2.CALIB_CB_ASYMMETRIC_GRID
    )
    if status:
        return centers
    else:
...
def calibrate_camera(img_pts, obj_pts, img_size):
    ...
    rms, camera_matrix, dist_coefs, rvecs, tvecs = cv2.calibrateCamera(
        obj_pts, img_pts, img_size, camera_matrix, dist_coef
    )
...
def pruning_quick_combine(l, fn, seed_idx=None, max_evals=1e20, max_depth=5):
    ...
    if seed_idx:
        ...
        seed_idx = range(
            len(l)
        )  # never happen, because we have an early exit if we have no seeds! patrick
        non_seed_idx = []
...
def get_system_info():
    ...
    return "User: {}, Platform: {}, Machine: {}, Release: {}, Version: {}".format(
        username, sysname, nodename, release, version
    )
