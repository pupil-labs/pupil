...
class Offline_Surface_Tracker(Surface_Tracker, Analysis_Plugin_Base):
    ...
    def recent_events(self,events):
        ...
        for s in self.surfaces:
            ...
            if s.detected:
                events['surfaces'].append({'name':s.name,'uid':s.uid,'img_to_surf_trans':s.m_to_screen.tolist(),'surf_to_img_trans':s.m_from_screen.tolist(),'gaze_on_surf': s.gaze_on_surf, 'timestamp':frame.timestamp,'camera_pose_3d':s.camera_pose_3d.tolist() if s.camera_pose_3d is not None else None})
        ...
        if self.mode == "Show marker IDs":
            ...
            if self.edit_surf_verts:
                ...
                # update surf with no or invald cache:
                for s in self.surfaces:
                    if s.cache == None:
                        s.init_cache(self.cache,self.min_marker_perimeter,self.min_id_confidence)
    ...
    def save_surface_statsics_to_file(self, export_range, export_dir):
        ...
        with open(os.path.join(metrics_dir,'surface_gaze_distribution.csv'),'w',encoding='utf-8',newline='') as csvfile:
            ...
            not_on_any_surf = set([gp['timestamp'] for gp in gaze_in_section])
            ...
            for s in self.surfaces:
                gaze_on_surf  = s.gaze_on_surf_in_section(section)
                gaze_on_surf = set([gp['base_data']['timestamp'] for gp in gaze_on_surf])
                not_on_any_surf -= gaze_on_surf
                csv_writer.writerow( (s.name, len(gaze_on_surf)) )

            csv_writer.writerow(('not_on_any_surface', len(not_on_any_surf) ) )
        ...
        with open(os.path.join(metrics_dir,'surface_events.csv'),'w',encoding='utf-8',newline='') as csvfile:
            ...
            csv_writer.writerow(('world_idx','world_timestamp','surface_name',
                                 'surface_uid','event_type'))

            events = []
            for s in self.surfaces:
                for enter_frame_id,exit_frame_id in s.cache.positive_ranges:
                    events.append({'frame_id':enter_frame_id,'surf_name':s.name,'surf_uid':s.uid,'event':'enter'})
                    events.append({'frame_id':exit_frame_id,'surf_name':s.name,'surf_uid':s.uid,'event':'exit'})

            events.sort(key=lambda x: x['frame_id'])
            for e in events:
                csv_writer.writerow( ( e['frame_id'],self.g_pool.timestamps[e['frame_id']],e['surf_name'],e['surf_uid'],e['event'] ) )
        ...
        for s in self.surfaces:
            ...
            with open(os.path.join(metrics_dir,'surf_positons'+surface_name+'.csv'),'w',encoding='utf-8',newline='') as csvfile:
                csv_writer =csv.writer(csvfile, delimiter=',')
                csv_writer.writerow(('world_idx','world_timestamp','img_to_surf_trans',
                                     'surf_to_img_trans','detected_markers'))
                for idx,ts,ref_surf_data in zip(range(len(self.g_pool.timestamps)),self.g_pool.timestamps,s.cache):
                    if in_mark <= idx < out_mark:
                        if ref_surf_data is not None and ref_surf_data is not False:
                            csv_writer.writerow( (idx,ts,ref_surf_data['img_to_surf_trans'],ref_surf_data['surf_to_img_trans'],ref_surf_data['detected_markers']) )
            ...
            with open(os.path.join(metrics_dir, 'gaze_positions_on_surface'+surface_name+'.csv'), 'w', encoding='utf-8', newline='') as csvfile:
                csv_writer = csv.writer(csvfile, delimiter=',')
                csv_writer.writerow(('world_timestamp', 'world_idx', 'gaze_timestamp',
                                     'x_norm', 'y_norm', 'x_scaled', 'y_scaled',
                                     'on_surf', 'confidence'))
                for idx, ts, ref_surf_data in zip(range(len(self.g_pool.timestamps)), self.g_pool.timestamps, s.cache):
                    if in_mark <= idx < out_mark:
                        if ref_surf_data is not None and ref_surf_data is not False:
                            for gp in s.gaze_on_surf_by_frame_idx(idx, ref_surf_data['surf_to_img_trans']):
            ...
            with open(os.path.join(metrics_dir,'fixations_on_surface'+surface_name+'.csv'),'w',encoding='utf-8',newline='') as csvfile:
                ...
                for idx,ref_surf_data in zip(range(len(self.g_pool.timestamps)),s.cache):
                    if in_mark <= idx < out_mark:
                        if ref_surf_data is not None and ref_surf_data is not False:
                            for f in s.fixations_on_surf_by_frame_idx(idx,ref_surf_data['surf_to_img_trans']):
                ...
                for f_on_s in removed_duplicates:
                    ...
                    csv_writer.writerow( (f['id'],f['timestamp'],f['duration'],f['start_frame_index'],f['end_frame_index'],f_x,f_y,f_x*s.real_world_size['x'],f_y*s.real_world_size['y'],f_on_surf) )
