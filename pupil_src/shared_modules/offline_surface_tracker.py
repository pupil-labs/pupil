...
class Offline_Surface_Tracker(Surface_Tracker, Analysis_Plugin_Base):
    ...
    def __init__(
        ...
        self.min_marker_perimeter_cacher = (
            20
        )  # find even super small markers. The surface locater will filter using min_marker_perimeter
    ...
    def load_marker_cache(self):
        ...
        if cache is None:
            ...
            self.invert_image = self.persistent_cache.get("inverted_markers", False)
            logger.debug(
                "Loaded marker cache {} / {} frames had been searched before".format(
    ...
    def recent_events(self, events):
        ...
        for s in self.surfaces:
            if not s.locate_from_cache(frame.index):
                s.locate(
                    self.markers, self.min_marker_perimeter, self.min_id_confidence
                )
            if s.detected:
