...
class Vis_Watermark(Plugin):
    ...
    def __init__(self, g_pool, selected_watermark_path=None, pos=(20, 20)):
        ...
        ]  # we only look for rgba images
        logger.debug(
            "Found {} watermark files: {}".format(
                len(self.available_files), self.available_files
            )
        )
        ...
        self.pos = list(
            pos
        )  # if we make the default arg a list the instance will edit the default vals and a new call of the class constructor creates an ainstace with modified default values.
    ...
    def load_watermark(self, path):
        ...
        if img is None:
            ...
            self.alpha_mask = np.dstack(
                (self.alpha_mask, self.alpha_mask, self.alpha_mask)
            )
    ...
    def on_click(self, pos, button, action):
        if self.move_watermark and action == 1:
            if (
                self.pos[0] < pos[0] < self.pos[0] + self.watermark.shape[0]
    ...
    def init_ui(self):
        ...
        if self.watermark is None:
            self.menu.append(
                ui.Info_Text(
                    "Please save a .png file in the users settings dir: '{}' in RGBA format. Once this plugin is closed and re-loaded the png will be used as a watermark.".format(
            ...
            if len(self.available_files) > 1:
                self.menu.append(
                    ui.Selector(
                        ...
                        self,
                        label="file",
                        selection=self.available_files,
                        labels=[os.path.basename(p) for p in self.available_files],
                        setter=self.load_watermark,
                    )
                )
