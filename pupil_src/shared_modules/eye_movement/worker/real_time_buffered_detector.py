...
import typing as t
import bisect
from eye_movement.utils import Gaze_Data, logger, can_use_3d_gaze_mapping, gaze_data_to_nslr_data
...
class Real_Time_Buffered_Detector:
    ...
    def segments_at_timestamp(self, target_timestamp: float) -> t.Iterable[Classified_Segment]:
    ...
    def segments_in_time_range(self, target_range: Time_Range) -> t.Iterable[Classified_Segment]:
        self._update_classification()
        return [segment for segment in self._segment_buffer if segment.time_range.intersection(target_range)]

    @staticmethod
    def _segment_generator(capture: Immutable_Capture, gaze_data: Gaze_Data, factory_start_id: int = None):
