...
def gaze_dispersion(capture, gaze_subset, use_pupil=True):
    if use_pupil:
        ...
        all_pp = chain.from_iterator((gp['base_data'] for gp in base_data))
...
class Offline_Fixation_Detector(Fixation_Detector_Base):
    ...
    def recent_events(self, events):
        if self.bg_task:
            ...
            if recent:
                ...
                self.fixations.extend(chain.from_iterator(data))
    ...
    def export_fixations(self, export_range, export_dir):
        ...
        fixations_in_section = chain.from_iterator(self.g_pool.fixations_by_frame[slice(*export_range)])
