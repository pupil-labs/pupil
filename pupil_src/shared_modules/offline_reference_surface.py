...
class Offline_Reference_Surface(Reference_Surface):
    ...
    def update_cache(
        self, marker_cache, min_marker_perimeter, min_id_confidence, idx=None
    ):
        """
        ...
        if self.cache is None:
            ...
            self.cache.update(
                ...
                self.answer_caching_request(
                    marker_cache, idx, min_marker_perimeter, min_id_confidence
                ),
            )
        else:
    ...
    def init_cache(self, marker_cache, min_marker_perimeter, min_id_confidence):
        if self.defined:
            ...
            self.cache = Cache_List(
                ...
                positive_eval_fn=lambda x: (x is not False) and (x is not None),
            )

    def answer_caching_request(
        self, marker_cache, frame_index, min_marker_perimeter, min_id_confidence
    ):
    ...
    def add_marker(
        ...
        super().add_marker(
            marker, visible_markers, min_marker_perimeter, min_id_confidence
        )
        self.cache = None
