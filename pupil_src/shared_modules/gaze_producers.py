...
def setup_fake_pool(
    frame_size, intrinsics, detection_mode, rec_dir, min_calibration_confidence
):
    cap = Empty()
...
colors = cycle(
    (
        (0.66015625, 0.859375, 0.4609375, 0.8),
...
class Gaze_Producer_Base(Producer_Plugin_Base):
    ...
    def init_ui(self):
        ...
        gaze_producer_plugins = [
            ...
            if issubclass(p, Gaze_Producer_Base)
        ]
...
class Gaze_From_Recording(Gaze_Producer_Base):
    def __init__(self, g_pool):
        ...
        self.recent_gaze_token = "Gaze_From_Recording"
        self.cache_dir = os.path.join(g_pool.rec_dir, "offline_data")
        os.makedirs(self.cache_dir, exist_ok=True)
        try:
            session_data = fm.load_object(
                os.path.join(self.cache_dir, "manual_gaze_correction.meta")
            )
    ...
    def cleanup(self):
        ...
        fm.save_object(
            session_data, os.path.join(self.cache_dir, "manual_gaze_correction.meta")
        )
...
class Offline_Calibration(Gaze_Producer_Base):
    ...
    session_data_name = "offline_gaze"
    ...
    def init_ui(self):
        ...
        def jump_next_natural_feature():
            ...
            for nf in self.manual_ref_positions:
                if nf["index"] > current:
                    self.notify_all(
                        {"subject": "seek_control.should_seek", "index": nf["index"]}
                    )
                    return
        ...
        self.menu.append(
            ui.Info_Text('"Detection" searches for circle markers in the world video.')
        )
        # self.menu.append(ui.Button('Redetect', self.start_marker_detection))
        slider = ui.Slider(
            "detection_progress", self, label="Detection Progress", setter=lambda _: _
        )
        slider.display_format = "%3.0f%%"
        self.menu.append(slider)

        toggle_label = (
            "Cancel circle marker detection"
            if self.process_pipe
            else "Start circle marker detection"
        )
        self.toggle_detection_button = ui.Button(
            toggle_label, self.toggle_marker_detection
        )
        ...
        self.menu.append(
            ui.Button(
                "Use calibration markers as natural features", use_as_natural_features
            )
        )
        self.menu.append(
            ui.Button("Jump to next natural feature", jump_next_natural_feature)
        )
        self.menu.append(
            ui.Switch("manual_ref_edit_mode", self, label="Natural feature edit mode")
        )
        self.menu.append(ui.Button("Clear natural features", clear_natural_features))

        self.menu.append(
            ui.Info_Text(
                "Calibration only considers pupil data that has an equal or"
                " higher confidence than the minimum calibration confidence."
            )
        )
        self.menu.append(
            ui.Slider(
    ...
    def append_section_menu(self, sec):
        ...
        section_menu.append(
            ui.Selector(
                "mapping_method", sec, label="Calibration Mode", selection=["2d", "3d"]
            )
        )
        section_menu.append(
            ui.Text_Input("status", sec, label="Calibration Status", setter=lambda _: _)
        )

        section_menu.append(
            ui.Info_Text(
                "This section is calibrated using reference markers found in a user"
                ' set range "Calibration". The calibration is used to map pupil to'
                ' gaze positions within a user set range "Mapping". Drag trim marks'
                " in the timeline to set a range and apply it."
            )
        )
        ...
        offset_menu.append(
            ui.Info_Text(
                "The manual correction feature allows you to apply"
    ...
    def on_click(self, pos, button, action):
        if action == glfw.GLFW_PRESS and self.manual_ref_edit_mode:
            manual_refs_in_frame = [
                r
                for r in self.manual_ref_positions
                if self.g_pool.capture.get_frame_index() == r["index"]
            ]
            for ref in manual_refs_in_frame:
                if (
                    np.sqrt(
                        (pos[0] - ref["screen_pos"][0]) ** 2
                        + (pos[1] - ref["screen_pos"][1]) ** 2
                    )
                    < 15
                ):  # img pixels
            ...
            new_ref = {
                ...
                "index_range": tuple(
                    range(
                        self.g_pool.capture.get_frame_index() - 5,
                        self.g_pool.capture.get_frame_index() + 5,
                    )
                ),
                "timestamp": self.g_pool.timestamps[
                    self.g_pool.capture.get_frame_index()
                ],
            }
        ...
        for sec in self.sections:
            if "bg_task" in sec:
                ...
                if sec["bg_task"].completed:
                    self.notify_all(
                        {
                            "subject": "offline_calibration.mapping_done",
                            "section": sec["label"],
                        }
                    )
                    del sec["bg_task"]
    ...
    def calibrate_section(self, sec):
        ...
        if not len(calibration_pupil_pos):
            logger.error(
                'No pupil data to calibrate section "{}"'.format(
        ...
        if (
            sec["mapping_method"] == "3d"
            and "2d" in calibration_pupil_pos[len(calibration_pupil_pos) // 2]["method"]
        ):
        ...
        logger.info(
            "Calibrating section {} ({}) in {} mode...".format(
    ...
    def gl_display(self):
        ...
        with gl_utils.Coord_System(0, 1, 0, 1):
            ref_point_norm = [
                r["norm_pos"]
                for r in self.circle_marker_positions
                if self.g_pool.capture.get_frame_index() == r["index"]
            ]
        ...
        self.timeline.content_height = max(
            0.001, self.timeline_line_height * len(self.sections)
        )
            ...
            for s in self.sections:
                ...
                if s["calibration_method"] == "natural_features":
                    ...
                    cygl_utils.draw_bars(
                        [(m["timestamp"], 0) for m in self.circle_marker_positions],
                        height=12 * scale,
                        thickness=scale,
                        color=color,
                    )
    ...
    def save_offline_data(self):
        ...
        for s in self.sections:
            sec = s.copy()
            for key in ("bg_task", "gaze", "gaze_ts", "gaze_topics"):
                if key in sec:
                    del sec[key]
            session_data["sections"].append(sec)
        session_data["pupil_token"] = self.recent_pupil_token
        session_data["gaze_token"] = self.recent_gaze_token
