...
class EyeSquare_Calibration(Calibration_Plugin):
    ...
    def start(self):
        ...
        if self.g_pool.detection_mapping_mode == '3d':
            ...
            if self.mode == 'calibration':
                ...
                self.sites = [(0.25,0), (0.75,0),
                              (0,0.25), (0.25,0.25), (0.5,0.25), (0.75,0.25), (1.0,0.25),
                              (0.25, 0.5), (0.75, 0.5),
                              (0, 0.75), (0.25, 0.75), (0.5, 0.75), (0.75, 0.75), (1.0, 0.75),
                              (0.25, 1.0), (0.75, 1.0)
                              ]
    ...
    def stop(self):
        ...
        if self.mode == 'calibration':
            ...
            matched_data = calibrate.closest_matches_monocular(self.ref_list, self.pupil_list)
            features = [x['pupil']['feature'] for x in matched_data]
            labels = [x['ref']['norm_pos'] for x in matched_data]
            self.g_pool.active_calibration_plugin.notify_all({'subject': 'start_plugin',
                                                              'name': 'Feature_Gaze_Mapper',
                                                              'args': {'train_features': features,
                                                                       'train_labels': labels}})

            ts = self.g_pool.get_timestamp()
            self.g_pool.active_calibration_plugin.notify_all(
                {'subject': 'calibration.successful', 'method': 'feature', 'timestamp': ts, 'record': True})
            self.g_pool.active_calibration_plugin.notify_all(
                {'subject': 'calibration.calibration_data', 'timestamp': ts, 'pupil_list': self.pupil_list,
                 'ref_list': self.ref_list,
                 'calibration_method': 'feature', 'record': True})
