...
class CalibrationV1:
    ...
    Result = namedtuple(
        "CalibrationResultV1", ["mapping_plugin_name", "mapper_args"]
    )
...
def _gazer_class_and_params_from_gaze_mapper_result(gaze_mapper_result):
    ...
    gaze_mapper_args = {k: v for k, v in gaze_mapper_args.items() if k not in deprecated_keys}

    if gaze_mapper_name == "Binocular_Vector_Gaze_Mapper":
        gazer_class = Gazer3D_v1x
