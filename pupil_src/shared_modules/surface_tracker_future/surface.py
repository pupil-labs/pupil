...
class Reference_Surface(object):
    ...
    def gl_display_heatmap(self):
        if self.detected:
            m = gl_utils.cvmat_to_glmat(self.m_surface_to_img_distorted)

            gl.glMatrixMode(gl.GL_PROJECTION)
            gl.glPushMatrix()
            gl.glLoadIdentity()
            gl.glOrtho(0, self.camera_model.resolution[0],
    ...
    def map_gaze_to_surface(self, gaze_data):
        ...
        gaze_points = np.array([methods.denormalize(g["norm_pos"],
