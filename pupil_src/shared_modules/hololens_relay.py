        ...
        while True:
            ...
            if ipc_sub.socket in items:
                while ipc_sub.new_data:
                    ...
                    # gaze events
                    if (
                        self.gaze_receiver is not None
                        and remote_socket is not None
                        and topic.startswith("gaze")
                    ):
                        ...
                        if method == "2d":
                            ...
                            logger.error(
                                'Error while relaying gaze: "{}": {}'.format(
                                    topic, payload
                                )
                            )
                    ...
                    # calibration events
                        ...
                        self.calib_result_receiver is not None
                        and remote_socket is not None
    ...
    def on_recv(self, socket, ipc_pub):
        ...
        if byte_msg[:1] == b"R":  # reference point
            ...
            try:
                ...
                ipc_pub.socket.send_string(
                    "notify.calibration.add_ref_data", flags=zmq.SNDMORE
                )
                ipc_pub.socket.send(byte_msg[1:])
            except Exception as e:
                # respond with <error byte><headercode byte>[<byte>, ...]
                response = (
                    b"FRReference point mal-formatted or missing: %s" % str(e).encode()
                )
            else:
                response = b"0R"
        elif byte_msg[:1] == b"S":
