...
class Pupil_Producer_Base(Producer_Plugin_Base):
    ...
    def _publish(self):
        self.notify_all(
            {"subject": "pupil_positions_changed", "token": self.recent_pupil_token}
        )
...
class Pupil_From_Recording(Pupil_Producer_Base):
    def __init__(self, g_pool):
        ...
        self.recent_pupil_token = "Pupil_From_Recording"
...
class Offline_Pupil_Detection(Pupil_Producer_Base):
    ...
    def __init__(self, g_pool):
        ...
        if self.eye_video_loc == [None, None]:
            self.correlate_publish_cache(token=session_meta_data["pupil_token"])
                ...
                if payload["subject"] == "file_source.video_finished":
                    for eyeid in (0, 1):
                        if self.eye_video_loc[eyeid] == payload["source_path"]:
                            ...
                            break
                    if self.eye_video_loc == [None, None]:
                        self.correlate_publish_cache(token=time.time())
    ...
    def correlate_publish_cache(self, token):
        self.recent_pupil_token = token
        self._correlate()
        self._publish()
        self.save_offline_data()

    def _correlate(self):
        timestamps = tuple(self.pupil_positions.keys())
        data = tuple(self.pupil_positions.values())
        topics = tuple(self.id_topics.values())
        self.g_pool.pupil_positions = pm.Bisector(data, timestamps)
